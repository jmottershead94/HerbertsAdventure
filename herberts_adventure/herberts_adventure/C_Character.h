...
class C_Character : public C_GameObject
{
	public:
		...
		C_Character();
		~C_Character();
		void Init(const int id, C_World* world, C_InputComponent* input, const std::string filename, const sf::Vector2f position, const float rotation, const sf::Vector2f scale);
