...
class C_Player : public C_Character
{
	public:
		...
		C_Player();
		~C_Player();
		void Init(const ObjectID id, std::shared_ptr<C_World> world, C_Camera* camera, const std::string filename, const sf::Vector2f position, const float rotation, const sf::Vector2f scale);
